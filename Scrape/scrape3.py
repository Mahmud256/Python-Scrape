...
url = f'https://shwapno.com/{category}'
driver.get(url)
...
products = soup.find_all('div', class_='product-box')  # Adjust this class if it differs

for product in products:
    title_tag = product.find('h2', class_="product-box-title")  # Adjust this class if it differs
    title = title_tag.get_text(strip=True) if title_tag else "No Title"

    dis_price_tag = product.find('span', class_="active-price")
    ...
    all_products.append({"Product Title": title, "Discount_price": discount_price})
